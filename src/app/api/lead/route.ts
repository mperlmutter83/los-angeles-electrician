...
const FROM = 'Los Angeles Electrician <info@yesidoinc.com>';
...
async function notifyWorkApplicant(opts: {
  ...
  const html = `
    <div style="font-family:system-ui,-apple-system,sans-serif;font-size:15px;color:#111;">
      <h2 style="margin:0 0 12px;">New work applicant — thelosangeleselectrician.com</h2>
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        subject: `New work applicant — ${opts.name} (thelosangeleselectrician.com)`,
        html,
      }),
    });
    if (!res.ok) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // WorkForm (looking-for-work) sends service: "Looking for Work".
    ...
    const parts = isWorkApplicant
      ? [
          'Service: Looking for Work',
          message ? `Details: ${message}` : null,
          'Source: thelosangeleselectrician.com',
        ]
      : [
    ...
    const serviceNeeded = parts.filter(Boolean).join(' | ') || null;
